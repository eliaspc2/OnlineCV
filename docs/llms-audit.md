...
Revisão da secção Perfil e dos cartões de capacidades: a narrativa passou de programação e engenharia de software para forma de trabalhar em operações e cibersegurança. Mantêm-se as experiências pessoais, agora associadas a planeamento, mudanças controladas, prevenção, integridade, comunicação, regras e continuidade de serviço. Os cartões de linguagens foram substituídos por um retrato do CET baseado no progresso comunicado pelo titular: base concluída em sistemas e redes, scripts e logs e segurança e conformidade; proteção e hardening em curso; vulnerabilidades e IDS por desenvolver. A tabela de progresso informa 636 horas de estudo em 1.025 horas curriculares (62,08%) em 6 de setembro de 2026; as 1.510 horas públicas incluem o estágio de 485 horas. A grelha de capacidades reserva duas linhas comuns para cada etiqueta em desktop, mantendo barras e estados alinhados entre si em todas as línguas. O cartão detalhado do CET usa a mesma distinção e apresenta os restantes itens apenas como conteúdos previstos. O cartão de Ciência de Dados usa agora o estado estável `Em curso`, mantendo a conclusão prevista, e apresenta a mesma estrutura dos restantes cartões: contexto, aplicação profissional em desenvolvimento, áreas do programa e tópicos relevantes previstos. A lista de Tecnologias Utilizadas mantém apenas tecnologias e bases já estudadas, retirando módulos de cibersegurança ainda futuros. O `llms.txt` foi revisto e continua factual. Versão `2026-09-06.11`, cache `json-site-v68`.
...
Limpeza adicional: removidas 7 referências de tradução órfãs por língua, relativas a ações flutuantes já retiradas. A verificação final encontrou 0 definições de objetos sem referência e 0 traduções órfãs. Nenhum facto público foi alterado.
